...
class vis_icarus(object):
    ...
    def get_trace_pmt(self,pmt_color='gray',pmt_size=6,pmt_opacity=0.5,pmt_colorscale=None,name=None):
        """
        ...
        hovertext = None
        try:
            hovertext = ['PMT %03d %.2f' % (i,float(v)) for i, v in enumerate(pmt_color)]
        except:
            pass
        try:
            pmt_size = float(pmt_size)
        except TypeError:
            pmt_size = np.array(pmt_size)
            pmt_size = (pmt_size - pmt_size.min())
            if pmt_size.max() > 0:
                pmt_size = pmt_size / pmt_size.max() * 8
            else:
                pmt_size[:]=6
    ...
    def get_trace_detector(self,draw_anode0=True,draw_anode1=True,draw_cathode0=True,draw_pmts=True,
                           pmt_value=None, pmt_size=6, pmt_opacity=0.5, pmt_colorscale=None):
        """
        ...
        if draw_pmts: trace += [self.get_trace_pmt(pmt_value,pmt_size,pmt_opacity,pmt_colorscale,name='PMTs')]
        return trace
